...
def mention_people(people, message, thread):
    ...
    if len(slack_ids) > 0:
        ...
        resp = api.send_slack_message(channel, message % mention_string)
        api.send_slack_message(channel, thread, thread_ts=resp["ts"])


api.sync_db_with_slack_and_return_count()

mention_people(
    floq_db.get_users_with_first_day(),
    "I dag har %s sin første dag i Blank!",
    "Velkommen 🌹",
)

mention_people(
    floq_db.get_users_with_birthday(), "I dag har %s bursdag!", "Gratulerer 🎈"
)
